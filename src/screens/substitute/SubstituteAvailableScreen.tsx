...
// ── Reason dropdown (Modal-based) ──────────────────────────────────────────

type BtnLayout = { x: number; y: number; width: number; height: number };

function ReasonDropdown({ value, onSelect }: { value: string; onSelect: (v: string) => void }) {
  const [open, setOpen] = useState(false);
    ...
    <View ref={btnRef}>
      ...
      <Modal visible={open} transparent animationType="none" onRequestClose={() => setOpen(false)}>
        <Pressable style={rd.backdrop} onPress={() => setOpen(false)}>
          {layout && (
            <View style={[rd.menu, { position: 'absolute', left: layout.x, width: layout.width, top: layout.y + layout.height }]}>
              {REASON_OPTIONS.map((opt) => (
                <TouchableOpacity
                  ...
                  activeOpacity={0.8}
                >
                  <Text style={[rd.optionText, opt === value && rd.optionTextActive]}>{opt}</Text>
                </TouchableOpacity>
              ))}
            </View>
          )}
...
function CalendarModal({
  ...
  visible: boolean; selected: string; onClose: () => void; onSelect: (dateStr: string) => void;
}) {
  ...
  const isSelected = (d: number) => !!parts && parts[0] === year && parts[1] === month + 1 && parts[2] === d;
      ...
      <Pressable style={cal.backdrop} onPress={onClose}>
        <Pressable style={cal.card} onPress={() => {}}>
          <View style={cal.header}>
            <TouchableOpacity onPress={prevMonth} hitSlop={12}><Text style={cal.navArrow}>‹</Text></TouchableOpacity>
            <Text style={cal.monthLabel}>{year}년 {MONTH_NAMES[month]}</Text>
            <TouchableOpacity onPress={nextMonth} hitSlop={12}><Text style={cal.navArrow}>›</Text></TouchableOpacity>
          </View>
          <View style={cal.dayRow}>
            {DAY_LABELS.map((d, i) => (
              <Text key={d} style={[cal.dayLabel, i === 0 && cal.sunText, i === 6 && cal.satText]}>{d}</Text>
            ))}
          </View>
          {weeks.map((week, wi) => (
              ...
              {week.map((day, di) => {
                ...
                return (
                  <TouchableOpacity
                    ...
                    activeOpacity={0.7}
                  >
                    <Text style={[cal.dayNum, di === 0 && cal.sunText, di === 6 && cal.satText, sel && cal.selNum, tod && cal.todayNum]}>
                      {day}
                    </Text>
                  </TouchableOpacity>
                );
...
const cal = StyleSheet.create({
  ...
  card:       { width: '100%', backgroundColor: '#FFFFFF', borderRadius: 20, padding: 20, shadowColor: '#000', shadowOffset: { width: 0, height: 8 }, shadowOpacity: 0.18, shadowRadius: 20, elevation: 12 },
...
export default function SubstituteAvailableScreen() {
  ...
  const handleSearch = async () => {
    if (!allFilled) { Alert.alert('입력 필요', '모든 항목을 기입해주세요.'); return; }
    if (!storeInfo) return;

    // YYYY.MM.DD → YYYY-MM-DD, HH:mm → HH:mm:00
    ...
    try {
      const res = await api.get(
        `/substitute/candidates?storeId=${storeInfo.storeId}&date=${requestDate}&startTime=${start}&endTime=${end}`
      );
      const raw = res.data.data?.candidates ?? [];
      const candidates = raw.map((c: any) => ({
        storeMemberId: c.storeMemberId,
        staffName: c.staffName ?? c.name ?? '?',
        availableTimes: c.availableTimes ?? [],
      }));
      if (candidates.length === 0) {
        Alert.alert('결과 없음', '해당 시간대에 대타 가능한 인원이 없습니다.\n대타 가능 시간대를 등록한 직원이 있어야 조회됩니다.');
        return;
      }
      (navigation as any).navigate('AiSubstituteList', { candidates, requestDate, startTime: start, endTime: end });
    } catch {
      Alert.alert('오류', '대타 후보 조회에 실패했습니다.');
    } finally {
      setSearching(false);
    }
  };
  ...
  return (
    <SafeAreaView style={s.safe}>
      ...
      >
        ...
        <View style={s.formCard}>
          ...
          <View style={[s.field, { zIndex: 15 }]}>
            ...
            <View style={s.timeRow}>
              <TimeDropdown value={startTime} options={TIMES} onSelect={setStart} placeholder="00:00" />
              <Text style={s.timeTilde}>~</Text>
              <TimeDropdown value={endTime} options={TIMES} onSelect={setEnd} placeholder="00:00" />
            </View>
          </View>
        ...
        {/* 대타 찾기 버튼 */}
        <TouchableOpacity
          style={[s.searchBtn, !!allFilled && !searching && s.searchBtnActive]}
        ...
        >
          {searching
            ...
            : <Text style={[s.searchBtnText, !!allFilled && s.searchBtnTextActive]}>대타 찾기</Text>
          }
        </TouchableOpacity>
      </ScrollView>
...
const s = StyleSheet.create({
  ...
  guideRow: {
    ...
    backgroundColor: '#FFF8F5', borderRadius: 8, paddingHorizontal: 10, paddingVertical: 8,
  },
